%fdm_plot.m 使用有限差分法求解二维空间域的抛物型方程绘制图像
%误差仅需考虑最后一个时间点的误差
clear; clc;
tic
%创建有限差分法求解类的实例
...
for n=1:Nt
    ...
    u_n=reshape(u_n',1,[])'; %将二维矩阵按行转换为列向量
...
toc
%% 计算误差
u_exact=zeros(Nx+1,Ny+1); %初始化解析解矩阵
for i=1:Nx+1
    for j=1:Ny+1
        u_exact(i,j)=fdm_obj.fun_analytical(t_test,x(i),y(j));
    end
end
error=max(max(abs(u-u_exact)));
fprintf('在时间点 t=%.2f 处的最大误差为: %.6e\n',t_test,error);
